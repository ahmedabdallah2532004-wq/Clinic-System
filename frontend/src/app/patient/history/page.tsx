...
export default function PatientHistory() {
  ...
  const combinedRecords = [
    ...encounters.map((e: any) => ({ 
      ...
      type: 'زيارة سريرية', 
      title: e.chiefComplaint || 'استشارة طبية',
      data: e 
    })),
    ...prescriptions.map((p: any) => ({ 
      ...
      type: 'وصفة طبية', 
      title: p.items?.[0]?.medicationName ? `${p.items[0].medicationName} وأخرى` : 'وصفة طبية علاجية',
      data: p 
    }))
  ...
  return (
    <DashboardLayout>
      <div className="mb-8 text-right" dir="rtl">
        <h1 className="text-3xl font-black tracking-tight">الأرشيف الطبي الكامل</h1>
        <p className="text-muted-foreground mt-1 font-medium">سجل زمني مفصل وتفصيلي لجميع زياراتك الطبية ووصفاتك العلاجية الصادرة.</p>
      </div>
      ...
      <div className="space-y-6" dir="rtl">
        {isLoading ? (
          ...
          <div className="py-20 text-center bg-accent/20 rounded-[2rem] border-2 border-dashed border-accent">
             ...
             <p className="text-lg font-bold text-muted-foreground">لا توجد سجلات طبية سابقة في الأرشيف الخاص بك.</p>
          </div>
        ) : (
          combinedRecords.map((record) => (
            <Card key={`${record.type}-${record.id}`} className="sub-border rounded-[2rem] group hover:border-primary/50 transition-all overflow-hidden shadow-sm text-right">
              <CardContent className="p-0">
                <div className="flex flex-col md:flex-row items-center gap-8 p-8">
                  <div className={cn(
                    ...
                    record.type === 'وصفة طبية' ? "bg-emerald-500 text-white shadow-emerald-200" : "bg-primary text-white shadow-primary-200"
                  )}>
                    {record.type === 'وصفة طبية' ? <FileText size={28} /> : <ClipboardList size={28} />}
                  </div>
                  ...
                  <div className="flex-1 text-center md:text-right">
                    <div className="flex flex-wrap justify-center md:justify-start items-center gap-3 mb-2">
                      <span className={cn(
                        ...
                        record.type === 'وصفة طبية' ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-primary/5 text-primary border-primary/10"
                      )}>
                        {record.type}
                      </span>
                      <span className="text-[10px] text-muted-foreground flex items-center gap-1.5 font-black uppercase tracking-tighter">
                        <Calendar size={12} className="text-primary" /> {format(new Date(record.date), 'dd MMMM yyyy', { locale: ar })}
                      </span>
                    </div>
                    ...
                    <p className="text-sm text-muted-foreground font-bold mt-1 flex items-center justify-center md:justify-start gap-1.5">
                      <User size={14} className="text-zinc-400" /> صدرت بواسطة د. {record.doctor}
                    </p>
                  </div>
                  ...
                  <div className="flex gap-3 w-full md:w-auto">
                    <Button 
                      ...
                      onClick={() => addToast('جاري فتح تفاصيل السجل الطبي...', 'info')}
                    >
                    ...
                    <Button 
                      ...
                      onClick={() => addToast('جاري تصدير التقرير بصيغة PDF...', 'info')}
                    >
                      <Download size={18} className="ml-2" /> تحميل
                    </Button>
                  </div>
                </div>
              </CardContent>
